...
func TestBackgroundNotTOTO(t *testing.T) {
	todo := fmt.Sprint(TODO())
	bg := fmt.Sprint(Background())

	if todo == bg {
		t.Errorf("TDDO and Backround are equal: %q vs %q", todo, bg)
	}
}
...
func TestWithDeadline(t *testing.T) {
	...
	if d, ok := ctx.Deadline(); !ok || d != deadline {
		t.Errorf("expected deadline %v, ot %v", deadline, d)
	}
	...
	if err := ctx.Err(); err != DeadlineExceeded {
		t.Errorf("err should be DeadlineExceeded, got %v", err)
	}
	...
	for _, tc := range tcs {
		...
		func() {
			...
			if val := ctx.Value(tc.keyRet); val != tc.valRet {
				t.Errorf("expected value %v, got %v", tc.valRet, val)
			}
		}()
		...
		if panicked == nil && tc.shouldPanic {
			t.Errorf("expected panic but did not get it")
		}
	}
}
